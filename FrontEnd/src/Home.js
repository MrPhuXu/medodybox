...
function Categories({ route, navigation }) {
  ...
  const getapiLoai = async () => {
    try {
      const response = await axios.get("http://192.168.2.109:3000/api/categories/getAll");
  ...
  const getFavoriteSongs = async () => {
    try {
      const response = await axios.get(
        "http://192.168.2.109:3000/api/playlists/My%20Favorite%20Songs/songs"
      );
  ...
  const getPopularSongs = async () => {
    try {
      const response = await axios.get(
        "http://192.168.2.109:3000/api/playlists/Viral%20Hits/songs"
      );
      setFavoriteSongs(response.data);
    } catch (error) {
      alert(error.message);
          ...
          renderItem={({ item }) => (
            <View>
              ...
              >
                <Image
                  source={{
                    uri: "http://192.168.2.109:3000/images/" + item.image,
                  }}
                  style={{
